...
func SetupWithDB(isCheckTx bool, patchGenesis func(*App, GenesisState) GenesisState, db dbm.DB) *App {
	encCfg := MakeEncodingConfig()
	app := NewApp(log.NewNopLogger(), db, DefaultNodeHome, nil, encCfg, DefaultOptions, baseapp.SetChainID(TestChainID))
	if !isCheckTx {
		// init chain must be called to stop deliverState from being nil
		genesisState := app.NewTestGenesisState(app.AppCodec())
		if patchGenesis != nil {
...
func (app *App) NewTestGenesisState(codec codec.Codec) GenesisState {
	privVal := mock.NewPV()
