...
func (suite *tallyHandlerSuite) TestVotePower_AllSourcesCounted() {
	...
	suite.Equal(sdkmath.NewInt(500e6+250e6+250e6), results.Yes)
	suite.Equal(sdkmath.ZeroInt(), results.No)
	suite.Equal(sdkmath.ZeroInt(), results.NoWithVeto)
	suite.Equal(sdkmath.ZeroInt(), results.Abstain)
}
